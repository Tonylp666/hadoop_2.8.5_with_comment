...
public class Cluster {
  ...
  private void initialize(InetSocketAddress jobTrackAddr, Configuration conf)
    ...
    initProviderList();
    /**
     *以迭代的方式去依次获取：ClientProtocolProvider
     *    ClientProtocolProvider：有两个实例：LocalClientProtocolProvider（本地运行）,
     *                                     YarnClientProtocolProvider（Yarn上运行）
     */
    for (ClientProtocolProvider provider : providerList) {
      ...
      try {
        if (jobTrackAddr == null) {
          //provider.create 其先跳到 LocalClientProtocolProvider
