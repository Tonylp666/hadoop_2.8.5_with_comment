...
class JobSubmitter {
  ...
  //liping:客户端通信协议实例,用于集群交互、完成作业提交、作业状态查询。
  ...
  throws ClassNotFoundException, InterruptedException, IOException {
    ...
    //liping:检查作业输出路径是否配置且已经存在。正确是：已配置但未存在，输出配置参数：mapreduce.output.fileoutputformat.outputdir
    checkSpecs(job);
    ...
    //有了这个目录，就可以将job运行所需的全部文件上传到对应的目录下了。
    ...
    try {
      ...
      LOG.debug("Configuring job " + jobId + " with " + submitJobDir 
          + " as the submit dir");
      // get delegation token for the dir,获取路径的授权
      ...
      //复制并配置相关文件，包括：-tmpfiles(外部文件)、tmpjars(第三方jar包)、tmparchives(一些归档文件)以及job.jar拷贝到HDFS中，
      // 这个方法最终调用jobClient.copyAndConfigureFiles(job, jobSubmitDir, replication);这个方法实现文件上传。
      ...
      // to job file.
      //首先获取队列名，没设置默认为:default,然后获取该队列的控制列表，并将队列以及控制列表存到conf
      ...
      /**
       * submitClient 方法提交Jobid，作业的路径，证书/令牌验证，返回状态
       * 此处的submitClient 就是上文的LocalJobRunner和YARNRunner
       */
  ...
  /**
   * 可以看出该方法首先获取splits数组信息后，排序，将会优先处理大文件。
   * JobSplitWriter.createSplitFiles(jobSubmitDir, conf, jobSubmitDir.getFileSystem(conf), array)方法会将split信息和SplitMetaInfo都写入HDFS中，其代码如下：
   */
