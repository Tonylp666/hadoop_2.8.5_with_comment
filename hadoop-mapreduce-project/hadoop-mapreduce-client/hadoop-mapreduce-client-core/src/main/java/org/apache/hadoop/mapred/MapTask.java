...
public class MapTask extends Task {
 ...
 private <T> T getSplitDetails(Path file, long offset) 
  ...
  void runNewMapper(final JobConf job,
    ...
    //制造切片，每一个切片对用一个Maptask,每个切片对应四部分(文件归属，偏移量，长度，位置信息)
    org.apache.hadoop.mapreduce.InputSplit split = null;
    split = getSplitDetails(new Path(splitIndex.getSplitLocation()),
        splitIndex.getStartOffset());
    LOG.info("Processing split: " + split);

    /*根据Inputformat对象创建RecordReader对象，默认是LineRecoedReader */
    ...
    if (job.getNumReduceTasks() == 0) {
      /*因为ReduceTasks == 0，因此没有reduce作业，直接将map结果输出到HDFS中。这里调用的是TextOutputFormat.getRecordWriter 这个方法首先会判断有没有压缩配置项。
      ...
      /* NewOutputCollector是有reducer的作业的map的输出。这个类的主要包含的对象是MapOutputCollector<K,V> collector = new MapOutputBuffer<K,V>(umbilical, job, reporter)，
      并且实例化了mapreduce.Partitioner<K,V> partitioner(默认是HashPartitioner.class) 这个是用来对mapper的输出数据进行分区的就是要数据要汇总到那个reducer上，
      NewOutputCollector的write方法会调用collector.collect(key, value,partitioner.getPartition(key, value, partitions))。*/
    ...
    try {
      /*初始化，在默认情况下调用的是LineRecordReader的initialize方法，主要是打开输入文件并且将文件指针指向文件头*/
  ...
  @InterfaceStability.Unstable
  /**
   * MapOutputBuffer Map任务暂存数据基类的缓冲区，大小有限。
   */
  public static class MapOutputBuffer<K extends Object, V extends Object>
    ...
    public void init(MapOutputCollector.Context context
      ...
      kvmeta = ByteBuffer.wrap(kvbuffer)
         ...
         .asIntBuffer();
      setEquator(0);//这个里面算好了 kvindex，初始化值为(kvbuffer.length - metasize)/4
      ...
      // compression
      //如果用户设置了codec 就用用户设置的，如果用户没有设置codec,就用 DefaultCodec.class
        ...
        try {
          do {
            if (!spillInProgress) {
              ...
              //(kvbend + METASIZE) % kvbuffer.length != equator - (equator % METASIZE)
              // 这里是说明已经发生过溢写操作（进行spill时，kvend会调整，equator会重新划分），而且程序可以进来，说明一处操作已经结束
              if ((kvbend + METASIZE) % kvbuffer.length !=
                  equator - (equator % METASIZE)) {
                // spill finished, reclaim space
                //liping  在这里，及那个bufstart,bufend重置为equator，kvstart\kvend 重置为第一条meta recode的开始位置
                resetSpill();
                //liping  重新计算bufferremaining
      ...
      try {
        ...
        for (int i = 0; i < partitions; ++i) {
          ...
          try {
            ...
            //如果设置了 combinerRunner ，则会进行combiner操作,相当于一次partition reduce。
            if (combinerRunner == null) {
              // spill directly
              ...
              while (spindex < mend &&
                ...
                /*liping  回收spill key/value 所占的空间*/
        ...
        //如果内存中索引大于1MB，则写到文件output/spill2.out.index中。
    ...
    /**
     * 该方法会将所有临时文件合并成一个大文件保存到output/file.out中，同时生成相应的索引文件output/file.out.index。
     * 在进行文件合并的过程中，Map Task以分区为单位进行合并。对于某个分区，它将采用多轮递归合并的方式：每轮合并io.sort.factor，默认是100，个文件，
     * 并将产生的文 件重新加入待合并列表中，对文件排序后，重复上述过程，直到只有一个文件。
     * 只生产一个文件可以避免同时打开大量的文件和同时读取大量的小文件产生的随机读 取带来的开销。最后会删除所有的spill文件。
     *
     * 另外需要注意的是，mergeParts()中也有combiner的操作，但是需要满足一定的条件：1、用户设置了combiner；
    ...
    private void mergeParts() throws IOException, InterruptedException, 
      ...
      }
      {
        ...
        IndexRecord rec = new IndexRecord();
        final SpillRecord spillRec = new SpillRecord(partitions);
        //循环所有分区，获得所有的spill文件的该分区数据，合并写入finalOut
