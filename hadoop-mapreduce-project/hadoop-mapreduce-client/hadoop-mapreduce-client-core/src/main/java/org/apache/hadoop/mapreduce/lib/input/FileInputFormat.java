...
public abstract class FileInputFormat<K, V> extends InputFormat<K, V> {
  ...
  public List<InputSplit> getSplits(JobContext job) throws IOException {
    ...
    for (FileStatus file: files) {
      ...
      if (length != 0) {
        ...
        if (file instanceof LocatedFileStatus) {
          ...
          //获所有块的block信息列表[hastname,offset,length]
          // 偏移量  block长度 block位置列表(0  134217728 node01,node02,node03)(134217728  48683115  node03,node04,node05)
          blkLocations = fs.getFileBlockLocations(file, 0, length);
        }
        //判断文件是否可分割，一般是可分割的，但是如果文件是压缩的，则不可分割。
        if (isSplitable(job, path)) {
          //获取块大小以及块的切片大小
          long blockSize = file.getBlockSize();
          //计算分片大小，Math.max(minSize, Math.min(maxSize, blockSize))
          long splitSize = computeSplitSize(blockSize, minSize, maxSize);

          long bytesRemaining = length;
          //liping  循环分片，注意这里有个 SPLIT_SLOP  1.1
          while (((double) bytesRemaining)/splitSize > SPLIT_SLOP) {//SPLIT_SLOP  1.1
            int blkIndex = getBlockIndex(blkLocations, length-bytesRemaining);//一开始：length = bytesRemaining，所以是从0开始
            splits.add(makeSplit(path, length-bytesRemaining, splitSize,
                        blkLocations[blkIndex].getHosts(),
                        blkLocations[blkIndex].getCachedHosts()));
            bytesRemaining -= splitSize;
          }
          //最后一块，为了避免最后一块太小哦啊，因此实际大小区间：[0,1.1*splitSize].
          if (bytesRemaining != 0) {
    ...
    // Save the number of input files for metrics/loadgen，设置文件数量。。
