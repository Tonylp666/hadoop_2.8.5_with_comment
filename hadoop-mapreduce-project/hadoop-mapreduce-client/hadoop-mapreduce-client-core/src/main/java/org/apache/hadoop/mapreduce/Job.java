...
public class Job extends JobContextImpl implements JobContext {  
  ...
  public void submit() 
         throws IOException, InterruptedException, ClassNotFoundException {
    ensureState(JobState.DEFINE);
    setUseNewAPI();//liping 使用新API
    //Connect to the Jobtracker and submit thr job
    /**
     *  3.初始化工作，为Cluster赋值，最主要的就是为Cluster中的Client赋值
     *  Client为提交器=>分为local提交器和Yarn提交器，具体创建哪个、由配置文件决定，下一个方法为connect()
     */
    connect();//生成JobClient对象，用来和JobTracker通信。
    //获取文件系统FileSystem和客户端Client
    ...
    status = ugi.doAs(new PrivilegedExceptionAction<JobStatus>() {
      public JobStatus run() throws IOException, InterruptedException, 
      ClassNotFoundException {
        return submitter.submitJobInternal(Job.this, cluster);//上传相关配置文件、提交
      }
    });
    state = JobState.RUNNING;
  ...
  /**
   * //liping:将作业提交到集群并等待他返回
   *
   * Submit the job to the cluster and wait for it to finish.
   * @param verbose print the progress to the user  将作业进度打印给用户
