...
public interface Decompressor {
   ...
   *
   * liping
   *      用未压缩的数据填充指定的缓冲区。返回未压缩数据的实际字节数。
   *      返回值0表示应该调用{@link #needsInput()}来确定是否需要更多的输入数据。
   *
   * @param b Buffer for the compressed data
   * @param off Start offset of the data
   * @param len Size of the buffer
   * @return The actual number of bytes of compressed data.---->即：解压后的数据大小。
   * @throws IOException
   */
  ...
  /**
   * liping :返回已压缩数据缓冲区（compressedDirectBuf）中剩余的字节数。
   *
