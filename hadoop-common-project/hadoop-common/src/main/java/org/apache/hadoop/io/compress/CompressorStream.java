...
public class CompressorStream extends CompressionOutputStream {
  ...
  public void write(byte[] b, int off, int len) throws IOException {
    // Sanity checks
    if (compressor.finished()) {//压缩器中没有数据
    ...
    /**
     *  往压缩器中塞数据
     *    这里应该是为了压缩效率，不是赛一点数据就压缩一点数据。
     *    而是当 uncompressedDirectBuf 中数据满了在执行压缩。
     */
  ...
  protected void compress() throws IOException {
    /**
     * liping 这个buffer 是输出的目标buffer-->compressedBuffer
     *        offset:compressedBuffer 的offset
     *
     */
  ...
  /**
   * liping
   *        当compressor 没有结束时，结束塞数据到压缩器，此时若compressor 仍然没有结束
   *        则，执行压缩流程，将已经塞到compressor 中的数据进行压缩操作。
   * @throws IOException
   */
  @Override
