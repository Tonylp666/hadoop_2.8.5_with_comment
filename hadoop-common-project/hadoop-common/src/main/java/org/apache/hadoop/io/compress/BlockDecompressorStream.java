...
public class BlockDecompressorStream extends DecompressorStream {
  //liping  originalBlockSize：原文数据块大小   noUncompressedBytes：目前已经取出的解压出的数据块大小。
  ...
  /**
   *  压缩的时候是按块进行压缩的，因此解压缩也是按块进行解压缩的。每次将一个数据块解压完，暂存在 decompressor 里面
   *  原文：      |++++++++++|++++++++++|++++++++++|++++++++++|...|++++++++++| 假设一个块100K
   *  按块解压缩后：|++++++++++| originalBlockSize = 100k
   *  取数据：     |++|         noUncompressedBytes = 2K
   *
   ...
   */
  @Override
  protected int decompress(byte[] b, int off, int len) throws IOException {//b:解压出来的数据
    ...
    int n = 0;
    //liping: n 是本次从解压缩机中取出的数据（解压出来的数据）
    while ((n = decompressor.decompress(b, off, len)) == 0) {
      if (decompressor.finished() || decompressor.needsDictionary()) {
        if (noUncompressedBytes >= originalBlockSize) {
          //liping  :解压出来的数据比原文大，肯定错了。返回-1
          eof = true;
          return -1;
        }
      }
      //liping: 当decompressor 需要输入时，首先调用 getCompressedData 获取数据，然后将数据塞进decompressor.
